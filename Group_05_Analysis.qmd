...
data5_glm <- glm(Total.Number.of.Family.members ~ 
                   log(Total.Household.Income) + 
                   log(Total.Food.Expenditure) + 
                   Household.Head.Sex + 
                   log(Household.Head.Age) + 
...
summary(data5_glm)
# table of coefficients summary
summary_table <- as.data.frame(summary(data5_glm)$coefficients)
kable(summary_table, "html", digits = 2)%>%
  kable_styling(font_size = 12,latex_options =c('scale_down','hold_position'))
```

since 1040 \< 1876.182, we don’t have evidence of lack of fit

```{r}
#check overdispersion of poission regression model
ggplot(data5_glm, 
       aes(x=fitted(data5_glm), 
...
```{r}
#check overdispersion of poission regression model
resp <- resid(data5_glm, type = "pearson")
resd <- resid(data5_glm, type = "deviance")
p1<- ggplot(data5_glm, aes(sample = resp)) + 
  geom_point(stat = "qq", color = "#7fc97f") +
ylab("Pearson residuals")
p2<- ggplot(data5_glm, aes(sample = resd)) + geom_point(stat = "qq", color = "#7fc97f") +
ylab("Deviance residuals")
p3<- ggplot(data5_glm, aes(x = predict(data5_glm, type="link"), y =resd))+
...
```{r}
#fit negetive binomial regression
data5_glm_nb <- glm.nb(Total.Number.of.Family.members ~ 
                   log(Total.Household.Income) + 
                   log(Total.Food.Expenditure) + 
                   Household.Head.Sex + 
                   Household.Head.Age + 
...
#summary of this model
summary(data5_glm_nb)
# table of coefficients summary
summary_table2 <- as.data.frame(summary(data5_glm_nb)$coefficients)
kable(summary_table2, "html", digits = 2)%>%
  kable_styling(font_size = 12,latex_options =c('scale_down','hold_position'))
```
